...
def pdf(testvar, sigma, mean):
    return 1/(np.sqrt(2 * np.pi * sigma **2)) * np.exp((-(testvar - mean) **2) / (2 * sigma **2))
...
evidence = (prior_probability_p * pdf(10, variance_x1_p, mean_x1_p) * pdf(5, variance_x2_p, mean_x2_p)) \
           + (prior_probability_q * pdf(10, variance_x1_q, mean_x1_q) * pdf(5, variance_x2_q, mean_x2_q))

posterior_p = (prior_probability_p * pdf(10, variance_x1_p, mean_x1_p) * pdf(5, variance_x2_p, mean_x2_p)) / evidence
print(posterior_p)

posterior_q = (prior_probability_q * pdf(10, variance_x1_q, mean_x1_q) * pdf(5, variance_x2_q, mean_x2_q)) / evidence
print(posterior_q)
